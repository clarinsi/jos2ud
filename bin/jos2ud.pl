...
sub feats_jos2ud {
    ...
    ##We strip features from $tmp_feats one by one, so same feat doesn't get processed twice
    ...
    while ($cont) {
        if ($tmp_feats =~ m/^\|*$/) {
            ...
            if (match_re($lemma, $m_lemma) && match_re($jos_cat, $m_cat) 
                && match_feats($tmp_feats, $m_feat)  ##&& match_feats($jos_feats, $m_feat)
                && match_re($ud_cat, $m_ud_cat)) {
                ##print STDERR "INFO: match $lemma to $feat_map[$i]\n";
                if ($tmp_feats eq '*') {$tmp_feats = ''}
                elsif ($m_feat ne '*') {
                    ##print STDERR "INFO: removing $m_feat from $tmp_feats\n";
                    $tmp_feats =~ s/$m_feat//
                }
